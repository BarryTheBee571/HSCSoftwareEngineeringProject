...
class LoginFrame(ctk.CTkFrame):
    ...
    def _btn(self, parent, text, cmd, fg=BLUE, text_color=WHITE, width=180, height=44):
        btn = ctk.CTkButton(
            ...
            font=("Trebuchet MS", self._s(14), "bold"),
            corner_radius=10, width=self._s(width), height=self._s(height),
        )
        self._scalable.append((btn, "Trebuchet MS", 14, "bold"))
        return btn

    def _entry(self, parent, placeholder, show=""):
        entry = ctk.CTkEntry(
            parent,
            font=("Trebuchet MS", self._s(13), "normal"),
            width=self._s(260), height=self._s(42),
        ...
        self._scalable.append((entry, "Trebuchet MS", 13, "normal"))
        return entry
    ...
    def _build(self):
        ...
        self._label(hdr, "Login or create an account to save your scores",
        ...
        self._login_tab_btn = ctk.CTkButton(
            ...
            hover_color=self._lighten(BLUE),
            font=("Trebuchet MS", self._s(13), "bold"),
            corner_radius=8, width=self._s(120), height=self._s(36),
            border_width=1, border_color=BLUE,
        )
        self._login_tab_btn.pack(side="left", padx=4)
        self._scalable.append((self._login_tab_btn, "Trebuchet MS", 13, "bold"))
        ...
        self._scalable.append((self._reg_tab_btn, "Trebuchet MS", 13, "bold"))
        ...
        self._btn(
            self, "Continue as Guest", self._guest_login,
    ...
    def _build_form(self):
        ...
        if self._mode == "login":
            ...
            btn_text   = "Log In  →"
            btn_cmd    = self._do_login
        else:
            title_text = "Create account"
            btn_text   = "Sign Up  →"
            btn_cmd    = self._do_register

        self._label(card, title_text, size=16, bold=True, color=BLUE).grid(
            row=0, column=0, pady=(18, 8))

        self._label(card, "Username", size=12, color=MUTED).grid(
            row=1, column=0, padx=30, pady=(4, 2), sticky="w")
        self.username_entry = self._entry(card, "Enter username")
        self.username_entry.grid(row=2, column=0, padx=30, pady=(0, 8))
        self.username_entry.bind("<Return>", lambda e: self.password_entry.focus())

        self._label(card, "Password", size=12, color=MUTED).grid(
            row=3, column=0, padx=30, pady=(4, 2), sticky="w")
        self.password_entry = self._entry(card, "Enter password", show="•")
        ...
        self._pw_toggle = ctk.CTkButton(
            ...
            width=self._s(140), height=self._s(24),
            corner_radius=4,
        )
        self._pw_toggle.grid(row=5, column=0, padx=30, pady=(0, 4), sticky="w")
        self._scalable.append((self._pw_toggle, "Trebuchet MS", 11, "normal"))

        self.msg_label = self._label(card, "", size=12, color=RED)
        self.msg_label.grid(row=6, column=0, pady=(2, 4))

        self._btn(card, btn_text, btn_cmd, width=200, height=44).grid(
