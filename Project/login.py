...
class LoginFrame(ctk.CTkFrame):
    ...
    def __init__(self, app):
        ...
        self.mode = "login"  # current tab: login/register
    ...
    def make_button(self, parent, text, cmd, fg=BLUE, text_color=WHITE, width=180, height=44):
        ...
        self.scalable.append((btn, "Comic Sans MS", 14, "bold"))  # track for resize
    ...
    def make_entry(self, parent, placeholder, show=""):
        ...
        self.scalable.append((entry, "Comic Sans MS", 13, "normal"))  # track for resize
    ...
    def setup_ui(self):
        ...
        self.scalable.append((self.login_tab_btn, "Comic Sans MS", 13, "bold"))
        ...
        self.reg_tab_btn.pack(side="left", padx=4)
        self.scalable.append((self.reg_tab_btn, "Comic Sans MS", 13, "bold"))
        ...
        self.form_card = card  # store to rebuild later
    ...
    def setup_login_form(self):
        ...
        card = self.form_card  # local alias
        card.grid_columnconfigure(0, weight=1)
        ...
        self.show_pw = False  # password hidden by default
        ...
        self.pw_toggle.grid(row=5, column=0, padx=30, pady=(0, 4), sticky="w")
        self.scalable.append((self.pw_toggle, "Comic Sans MS", 11, "normal"))
    ...
    def toggle_password(self):
        ...
        self.show_pw = not self.show_pw  # flip visibility flag
        if self.show_pw:
            self.password_entry.configure(show="")  # show real text
            self.pw_toggle.configure(text="Hide password")  # update button label
        else:
            self.password_entry.configure(show="*")  # mask text again
            self.pw_toggle.configure(text="Show password")  # update button label

    def switch_mode(self, mode):
        """Switch between login and register tabs."""
        self.mode = mode  # save selected tab
        if mode == "login":
            self.login_tab_btn.configure(fg_color=BLUE, text_color=WHITE)  # active style
            self.reg_tab_btn.configure(fg_color=WHITE, text_color=BLUE)  # inactive style
        else:
            self.login_tab_btn.configure(fg_color=WHITE, text_color=BLUE)  # inactive style
            self.reg_tab_btn.configure(fg_color=BLUE, text_color=WHITE)  # active style
